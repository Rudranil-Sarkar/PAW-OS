...
char decode_scancode(u8 scancode)
{
    ...
    if (scancode == 0xE0) { _MULTIMEDIA_PREFIX = 1; return -0x02; }

    else if (_MULTIMEDIA_PREFIX) {
        if (scancode == 0x49) {
            return KEY_PAGE_UP;
        }
        _MULTIMEDIA_PREFIX = 0;
    }

    else if(scancode == 0x3A) { _CAPS_LOCK_STATUS = !_CAPS_LOCK_STATUS; return -0x02; }
    else if(scancode == 0x2A) { _SHIFT_STATUS = 1; return -0x02; }
